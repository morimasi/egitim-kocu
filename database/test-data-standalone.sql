-- Eğitim Koçluğu Uygulaması Test Verileri (Auth Bağımsız)
-- Bu dosyayı Supabase SQL Editor'de çalıştırarak test verileri ekleyebilirsiniz

-- GEÇICI: Auth bağımsız test için RLS'yi devre dışı bırak
...
-- Profiles tablosundaki foreign key kısıtlamasını geçici olarak kaldır
...
-- Verilerin başarıyla eklenip eklenmediğini kontrol et
SELECT 
...
SELECT 'STUDENTS WITH COACH:' as info;
SELECT 
    s.grade_level,
    s.school_name,
