...
export default async function StudentInboxPage() {
  ...
  // Kök mesajlar (yeni konuşmalar) – parent_message_id null
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Koçunuzla mesajlaşmalar</CardDescription>
          </CardHeader>
          <CardContent>
            {(!threads || threads.length === 0) ? (
              ...
              <ul className="divide-y divide-gray-200 bg-white shadow sm:rounded-md overflow-hidden">
                {threads.map((t: any) => {
                  ...
                  return (
                    <li key={t.id}>
                      <Link href={`/dashboard/student/messages/${t.id}`} className="block">
